...
public class RealWorldTerrainGrassGenerator : EditorWindow
{
    private static RealWorldTerrainGrassGenerator wnd;

    private List<RealWorldTerrainGeneratorColor> colors;
    ...
    private void ExportSettings()
    {
        string path = EditorUtility.SaveFilePanel("Export settings", Application.dataPath,
            "GrassGeneratorSettings.xml", "xml");
        if (string.IsNullOrEmpty(path)) return;

        XmlDocument doc = new XmlDocument();
        XmlElement firstElement = (XmlElement) doc.AppendChild(doc.CreateElement("GrassGenerator"));
        firstElement.SetAttribute("dryColor", dryColor.ToHex());
        firstElement.SetAttribute("healthyColor", healthyColor.ToHex());
        firstElement.SetAttribute("minWidth", minWidth);
        firstElement.SetAttribute("maxWidth", maxWidth);
        firstElement.SetAttribute("minHeight", minHeight);
        firstElement.SetAttribute("maxHeight", maxHeight);
        firstElement.SetAttribute("noiseSpread", noiseSpread);
        firstElement.SetAttribute("textureID", (texture != null) ? texture.GetInstanceID() : -1);

        foreach (RealWorldTerrainGeneratorColor color in colors) firstElement.AppendChild(color.GetNode(doc));
...
// ReSharper disable once UnusedMember.Local
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("Add color")) colors.Add(new RealWorldTerrainGeneratorColor());
    ...
    {
        if (colors == null) colors = new List<RealWorldTerrainGeneratorColor>();
    ...
    {
        wnd = GetWindow<RealWorldTerrainGrassGenerator>("Generator", true);
        wnd.item = item;
    }
}