...
public class KaijuMain : MonoBehaviour {
    ...
    System.Collections.Generic.List<string> PNameArray = new System.Collections.Generic.List<string>();  // array of player names
    ...
    ///////////// ADD TARGET OBJECTS BY LAT AND LONG /////////////////////////////////////////
    ...
    void ParseJson()
    {
        ...
        for (int i = 0; i < tLength; i++){
            ...
            // Debug.Log("NAME IS: " + tName.Substring(24));
            
            PNameArray.Add(tName.Substring(24));
    ...
    void addTargets(){
        ...
        for(int i = 0; i<tLat.Length; i++){
            ...
            /// latLongToXZ(float.Parse(tLat[i]), float.Parse(tLong[i]), PNameArray[i]);
    ...
    void latLongToXZ(float tLat, float tLong, string tName){
        ...
        /// converts the latlong data from the actual RWT positioning to x,z in Unity Units using the proportions generated by the mapWidth/mapHeight placeholder
        ...
        GameObject GameTarget = GameObject.Instantiate(playerMarker);

        GameTarget.AddComponent<Rigidbody>();
        GameTarget.transform.position = newPos;
        var data = GameTarget.GetComponent<TargetProfileData>();
        string shortName = tName.Substring(24);
        data.TargetName = shortName;
        data.TargetId = tName;
        // PNameArray.Add(tName);

        var label = GameTarget.transform.GetComponentInChildren<UnityEngine.UI.Text>();
        if(label != null)
        {
